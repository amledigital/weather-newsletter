...
var tmplFuncs = template.FuncMap{
	...
	"convertToF": func(temp float64) int {
		result := (temp * 9 / 5) + 32
		return int(result)
	},
}
...
func (m Mailer) Send(recipient, templateFile string, data any) error {
	...
	// file system.
	tmpl, err := template.New("email").Funcs(tmplFuncs).ParseFS(templateFS, "templates/"+templateFile)
	if err != nil {
		return err
	}
	...
	msg.SetHeader("X-PM-Message-Stream", "broadcast")
